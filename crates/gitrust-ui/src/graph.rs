...
pub fn render_graph_cell(row: &RowLayout, width: usize) -> Element {
    ...
    // The CSS forces log rows to a single line via `white-space: nowrap`
    // on the summary column, so each row settles near 28-32px tall. The
    // vertical halves overhang the viewBox by `OVERHANG` on each side to
    // bridge whatever 2-4px the table-cell padding leaves around the SVG.
    // Diagonals stay inside the box so they don't visually overshoot the
    // lane they merge into.
